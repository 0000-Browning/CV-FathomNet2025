...
annotations_path = "~/CS/kaggle/fathomnet-2025/train/annotations.csv"
    ...
    for attempt in range(retries):
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"Failed to fetch rank for {label}. Status code: {response.status_code}"
                )
...
labels_with_ranks = pd.DataFrame(
    {"Label": list(taxonomic_ranks.keys()), "Rank": list(taxonomic_ranks.values())}
)
...
labels_with_ranks["Rank"] = pd.Categorical(
    labels_with_ranks["Rank"], categories=rank_order, ordered=True
)
...
output_path = (
    r"c:\Users\jake3\Desktop\CV Repo\CV-FathomNet2025\sorted_labels_with_ranks.csv"
)
